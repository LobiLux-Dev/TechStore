...
export function CategoriesSection() {
  ...
  const handleExportExcel = async () => {
    try {
      ...
      const categoriesToExport = data?.categories || [];
      ...
      toast.success("Categorías exportadas a Excel");
    } catch (err: any) {
  ...
  const handleSaveCreate = (e: React.FormEvent) => {
    ...
    createCategory({
      variables: { input: { name: categoryName.trim() } },
    });
  };
  ...
  const totalCount = (data as any)?.totalCategories || 0;
  ...
  const columns = useMemo(() => [
    ...
    columnHelper.accessor("name", {
      header: () => (
        ...
        >
          Nombre Categoría {orderBy === "name" && (sortOrder === "asc" ? "▲" : "▼")}
        </span>
      ),
      cell: (info) => (
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-border/60 pb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Organiza tus productos en clasificaciones lógicas.
          </p>
        </div>
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between px-6 py-3 border-t border-border/40">
              <span className="text-[11px] font-mono text-muted-foreground">
                Página {page + 1} de {totalPages} ({totalCount} categorías)
              </span>
              <div className="flex gap-2">
                <Button
                  variant="outline"
      ...
      <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
        <DialogContent className="sm:max-w-[400px] border-border bg-card/95 backdrop-blur-lg">
          <form onSubmit={handleSaveCreate}>
            <DialogHeader>
              <DialogTitle className="font-heading font-bold text-foreground">Crear Categoría</DialogTitle>
              <DialogDescription className="text-xs">
                Añade una nueva clasificación para agrupar tus productos.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="flex flex-col gap-1 py-4">
              ...
              <Input
                ...
                placeholder="ej. Componentes, Periféricos, Laptops"
                className="rounded-lg"
              />
            </div>
      ...
      <Dialog open={isEditOpen} onOpenChange={setIsEditOpen}>
        <DialogContent className="sm:max-w-[400px] border-border bg-card/95 backdrop-blur-lg">
          <form onSubmit={handleSaveEdit}>
            <DialogHeader>
              <DialogTitle className="font-heading font-bold text-foreground">Editar Categoría</DialogTitle>
              <DialogDescription className="text-xs">
      ...
      <Dialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <DialogContent className="sm:max-w-[400px] border-border bg-card/95 backdrop-blur-lg">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              ¿Estás seguro de eliminar la categoría <strong className="text-foreground">"{selectedCategory?.name}"</strong>? Los productos asociados perderán su categoría asignada.
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="gap-2 sm:gap-0 mt-4">
            ...
            <Button
              ...
              onClick={() => {
                if (selectedCategory) {
                  deleteCategory({
                    variables: { id: parseInt(selectedCategory.id, 10) },
                  });
                }
              }}
              className="rounded-lg"
